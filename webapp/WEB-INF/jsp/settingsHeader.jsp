...
<c:choose>
    <c:when test="${empty user or user.adminRole}">
        <c:set var="categories" value="musicFolder general advanced personal user player network share premium dlna sonos transcoding internetRadio podcast videoConversion"/>
    </c:when>
    <c:otherwise>
...
<c:forTokens items="${categories}" delims=" " var="cat" varStatus="loopStatus">
    ...
    <span class="${category eq cat ? 'headerSelected' : 'header'}"><a href="${url}"><fmt:message key="settingsheader.${cat}"/></a></span>
</c:forTokens>
</h2>
