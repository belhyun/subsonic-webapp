...
<script type="text/javascript" language="javascript">
    ...
    function loadAlbumInfo() {
        multiService.getAlbumInfo(${model.dir.id}, 8, function (albumInfo) {
            ...
            if (albumInfo.notes) {
                $("#artistBio").append(albumInfo.notes + " Image courtesy of fanarttv.com.");
                $("#albumInfoTable").show();
            }
        });
    }
...
<c:if test="${not model.partyMode}">
    <h2>
        ...
        <c:if test="${model.user.streamRole}">
            ...
            <span class="header"><a href="javascript:playRandom()"><fmt:message key="common.shuffle"/></a></span> |
            <span class="header"><a href="javascript:addAll()"><fmt:message key="common.add"/></a></span>
            <c:set var="needSep" value="true"/>
        </c:if>
            ...
            <table class="music">
                ...
                <c:forEach items="${model.files}" var="song" varStatus="loopStatus">
                    ...
                    <tr style="margin:0;padding:0;border:0">
                        ...
                        <td class="fit center"><input type="checkbox" class="checkbox song-checkbox" id="songIndex${loopStatus.count - 1}">
                ...
                <c:if test="${model.viewAsList}">
                    <c:forEach items="${model.sieblingAlbums}" var="album" varStatus="loopStatus">
                        <tr>
                            ...
                            <td class="fit right detail">${album.year}</td>
                        </tr>
                    </c:forEach>
                </c:if>
            </table>
